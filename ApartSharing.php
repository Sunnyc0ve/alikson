<?php
$title='Alikson Apart Sharing';
$csspage='_apartsharing.css?2.6'; 
include('include/header.php');
?>
<main>
<section class="MainSection">
<div class="MainSection-Header">
<h1>apart sharing</h1>
<p class="p_desktop">Инновационная платформа для&nbsp;аренды, сдачи и&nbsp;управления недвижимостью, который автоматизирует процесс и&nbsp;делает его максимально удобным для&nbsp;арендодателей и&nbsp;арендаторов. Предлагаем современные решения для&nbsp;бесконтактного заселения, контроля объектов и&nbsp;управления доходами</p>
<p class="p_mobile">Инновационная платформа для&nbsp;аренды, сдачи и&nbsp;управления недвижимостью, который автоматизирует процесс и&nbsp;делает его максимально удобным для&nbsp;арендодателей и&nbsp;арендаторов</p>
</div>
<div class="MainSection-Content">
<a href="">Оставить заявку</a>
<div>
<div class="Info">
<div>
<h3>5&nbsp;000+</h3>
<p>клиентов&nbsp;&mdash; ежемесячный прирост аудитории</p>
</div>
</div>
<div class="Info">
<div>
<h3>93%</h3>
<p>клиентов пользуются сервисом на&nbsp;постоянной основе</p>
</div>
</div>
<div class="Info">
<div>
<h3>&gt;24&nbsp;000</h3>
<p>новых пользователей установили приложение за&nbsp;последние 6&nbsp;мес.</p>
</div>
</div>
</div>
</div>
</section>
<section class="ServicesSection">
<div>
<h2>Наши сервисы</h2>
<div class="Services-Content">
<div class="Card">
<div> 
<img src="img/apart_sharing/Service1.svg">
</div>
<h3>Интерактивная карта объектов</h3>
</div>
<div class="Card">
<div> 
<img src="img/apart_sharing/Service2.svg">
</div>
<h3>Интеграция с&nbsp;платформами бронирования</h3>
</div>
<div class="Card">
<div> 
<img src="img/apart_sharing/Service3.svg">
</div>
<h3>Управление объектами</h3>
</div>
<div class="Card">
<div> 
<img src="img/apart_sharing/Service4.svg">
</div>
<h3>Автоматизация процессов</h3>
</div>
</div>
</div>
</section>
<section class="AutoSection">
<h2>Система<br> автоматизации</h2>
<div class="Auto-Content">
<div class="Card">
<h3>Apart Sharing</h3>
<p>Приложение для&nbsp;клиентов</p>
<ul class="Card-Content">
<li>выбор квартир на&nbsp;карте</li>
<li>бронирование и&nbsp;оплата</li>
<li>открытие домофона</li>
<li>закрытие&nbsp;/ открытие квартиры</li>
<li>завершение аренды</li>
<li>участие в&nbsp;реферальной программе</li>
<li>участие в&nbsp;бонусных программах</li>
<li>получение выплат и&nbsp;бонусов</li>
</ul>
</div>
<div class="Card">
<h3>Apart Business</h3>
<p>Приложение для&nbsp;франчайзи</p>
<ul class="Card-Content">
<li>добавление квартир</li>
<li>отслеживание бронирований</li>
<li>получение выплат</li>
<li>мониторинг объекта</li>
<li>финансовая статистика</li>
</ul>
</div>
<div class="Card">
<h3>Apart Cleaning</h3>
<p>Приложение для&nbsp;сотрудников</p>
<ul class="Card-Content">
<li>получение заявок на&nbsp;уборку</li>
<li>выбор графика работы</li>
<li>отправление отчетов о&nbsp;состоянии квартиры</li>
<li>получение заработной платы</li>
</ul>
</div>
</div>
<div class="Auto-Content_mobile">
<div class="swiper">
<div class="swiper-wrapper">
<div class="swiper-slide">
<div class="Card">
<h3>Apart Sharing</h3>
<p>Приложение для&nbsp;клиентов</p>
<ul class="Card-Content">
<li>выбор квартир на&nbsp;карте</li>
<li>бронирование и&nbsp;оплата</li>
<li>открытие домофона</li>
<li>закрытие&nbsp;/ открытие квартиры</li>
<li>завершение аренды</li>
<li>участие в&nbsp;реферальной программе</li>
<li>участие в&nbsp;бонусных программах</li>
<li>получение выплат и&nbsp;бонусов</li>
</ul>
</div>
</div>
<div class="swiper-slide">
<div class="Card">
<div class="Card-Header">
<h3>Apart Business</h3>
<p>Приложение для&nbsp;франчайзи</p>
</div>
<ul class="Card-Content">
<li>добавление квартир</li>
<li>отслеживание бронирований</li>
<li>получение выплат</li>
<li>мониторинг объекта</li>
<li>финансовая статистика</li>
</ul>
</div>
</div>
<div class="swiper-slide">
<div class="Card">
<h3>Apart Cleaning</h3>
<p>Приложение для&nbsp;сотрудников</p>
<ul class="Card-Content">
<li>получение заявок на&nbsp;уборку</li>
<li>выбор графика работы</li>
<li>отправление отчетов о&nbsp;состоянии квартиры</li>
<li>получение заработной платы</li>
</ul>
</div>
</div>
</div>
<div class="Swiper-Control">
<button class="Swiper-Prev">
<img src="img/ArrowLeft.svg" />
</button>
<button class="Swiper-Next">
<img src="img/ArrowRight.svg" />
</button>
</div>
</div>
</div>
</section>
<section class="AdvantagesSection">
<div class="Advantages-Container">
<div class="Advantages-Header">
<h2>Преимущества Apart Sharing</h2>
</div>
<div class="Advantages-Content">
<div class="Card">
<img src="img/apart_sharing/checkbox.svg"> 
<p>Высокая доходность</p>
</div>
<div class="Card">
<img src="img/apart_sharing/checkbox.svg">
<p>Снижение расходов на&nbsp;привлечение гостей</p>
</div>
<div class="Card">
<img src="img/apart_sharing/checkbox.svg">
<p>Безопасный прием платежей</p>
</div>
<div class="Card">
<img src="img/apart_sharing/checkbox.svg">
<p>Бесконтактное заселение</p>
</div>
<div class="Card">
<img src="img/apart_sharing/checkbox.svg">
<p>Обязательная верификация гостей</p>
</div>
<div class="Card">
<img src="img/apart_sharing/checkbox.svg">
<p>Мобильное приложение</p>
</div>
</div>
</div>
</section>
<section class="HowSection">
<div class="How-Header">
<h2>Как это работает?</h2>
</div>
<div class="How-Content">
<div class="Card">
<h3>01</h3>
<p>Владелец регистрируется в&nbsp;приложении Apart Business и&nbsp;загружает информацию о&nbsp;своей недвижимости</p>
</div>
<div class="Card">
<h3>02</h3>
<p>Платформа автоматически управляет бронированиями, заселением и&nbsp;списанием оплаты</p>
</div>
<div class="Card">
<h3>03</h3>
<p>Владелец может отслеживать доход, статистику по&nbsp;бронированиям и&nbsp;состояние объекта в&nbsp;режиме реального времени</p>
</div>
<div class="Card">
<h3>04</h3>
<p>Мобильное приложение Apart Cleaning позволяет автоматизировать процессы уборки и&nbsp;контроля качества после каждого арендатора</p>
</div>
</div>
</section>
<?php include("include/form.php") ?>
</main>
<?php include('include/footer.html') ?>
</body>
<script src="include/header.js"></script>
<script>
    const swiper = new Swiper(".swiper", {
      slidesPerView: 1,
      spaceBetween: -340,
      watchSlidesProgress: true, // Следит за прогрессом слайдов
  watchSlidesVisibility: true, // Следит за видимостью слайдов
      direction: "horizontal",
      loop: false,
      navigation: {
        nextEl: ".Swiper-Next",
        prevEl: ".Swiper-Prev",
      },
      breakpoints: {
        660: {slidesPerView: 2, spaceBetween: -350 },
      },
    });
  </script>
</html>